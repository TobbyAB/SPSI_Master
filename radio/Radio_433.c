...
{
    rt_pin_mode(IRQ1, PIN_MODE_INPUT_PULLUP);
    rt_pin_attach_irq(IRQ1, PIN_IRQ_MODE_RISING, IRQ1_ISR, RT_NULL);
    rt_pin_irq_enable(IRQ1, PIN_IRQ_ENABLE);
}
struct ax5043_config *rf_433_config_init(void)
{
    ...
    config->axradio_phy_chanfreq[0] = 0x10b72373;
    ...
    while(1)
    {
        ...
        if (result == RT_EOK)
        {
            ...
            case trxstate_rx: //0x01
                ...
                if (rf_433.RxLen != 0)
                {
                    if (rf_433.ubRssi > -26 && rf_433.ubRssi < 0 )
                    {
