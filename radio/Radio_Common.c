...
void InitAx5043REG(struct ax5043 *dev)
{
    ...
    SpiWriteLongAddressRegister(dev, REG_AX5043_PKTLENOFFSET,
            (SpiReadLongAddressRegister(dev, REG_AX5043_PKTLENOFFSET)));
...
void TransmitData(struct ax5043 *dev)
{
    ...
    while (1)
    {
        ...
        if (SpiReadSingleAddressRegister(dev, REG_AX5043_FIFOFREE1))
            ubFreeCnt = 0xff;
        ...
        case trxstate_tx_shortpreamble:   //B
            shortpreamble: if (dev->axradio_txbuffer_cnt == 0)
            {
                if (ubFreeCnt < 15)
                    goto fifocommit;
                ...
                if ((SpiReadSingleAddressRegister(dev, REG_AX5043_FRAMING) & 0x0E) == 0x06
                        && dev->config->axradio_framing_synclen)                            //axradio_framing_synclen=32
                {
                    ...
                    for (i = 0; i < len_byte; ++i)
                    {
                        ...
                        SpiWriteSingleAddressRegister(dev, REG_AX5043_FIFODATA,
                                dev->config->axradio_framing_syncword[i]);
                    }
    ...
    fifocommit: SpiWriteSingleAddressRegister(dev, REG_AX5043_FIFOSTAT, 4); // commit
    pktend: SpiWriteSingleAddressRegister(dev, REG_AX5043_FIFOSTAT, 4); // commit
...
int16_t axradio_tunevoltage(struct ax5043 *dev)
{
    ...
    do
    {
        ...
        do
        {
        } while (SpiReadLongAddressRegister(dev, REG_AX5043_GPADCCTRL) & 0x80);
    } while (--cnt);
    cnt = 32;
    do
    {
...
void PLLRang_wait_for_xtal(struct ax5043 *dev)
{
    ...
    SpiWriteSingleAddressRegister(dev, REG_AX5043_IRQMASK1,
            SpiReadSingleAddressRegister(dev, REG_AX5043_IRQMASK1) | 0x01); // enable xtal ready interrupt
    for (;;)
    {
...
//    IWDG_ReloadCounter();
    ...
    if (RangeMin == 0)
        SpiWriteLongAddressRegister(dev, REG_AX5043_PLLVCOI, 0xbe);
    else
